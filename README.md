# Observer.CLI

---

Easily acquire economic data from the St.Louis Federal Reserve [FRED® API](https://fred.stlouisfed.org/docs/api/fred/) and store it in a database on your local machine.

Before using this software you must [request an API key from FRED®](https://fred.stlouisfed.org/docs/api/api_key.html).

----

[Read the documentation here](https://vyntix.com/docs/fred-client/latest/intro.html).

---


##### This software uses the FRED® API but is not endorsed or certified by the Federal Reserve Bank of St. Louis.

##### By using this software you agree to be bound by the [FRED® API Terms of Use](https://fred.stlouisfed.org/legal/).


 ---
